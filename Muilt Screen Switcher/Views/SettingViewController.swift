...
class SettingViewController: NSViewController ,NSTableViewDataSource,NSTableViewDelegate{
    ...
    func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView?{
        if tableView.tag == 1{
            let cell = tableView.make(withIdentifier: "profileCell", owner: nil) as! NSTableCellView
            cell.textField?.bind("value", to: _profile[row], withKeyPath: "note", options: [NSContinuouslyUpdatesValueBindingOption: true])
            return cell
            ...
            if tableColumn == displayTable.tableColumns[0]{
                // Display id
                let cell = displayTable.make(withIdentifier: "displayid", owner: nil) as! NSTableCellView
                cell.textField?.stringValue = String(displays[row].screen_id)
                return cell
                
            }else if tableColumn == displayTable.tableColumns[1]{
                // Position
                let cell = displayTable.make(withIdentifier: "displayPosition", owner: nil) as! NSTableCellView
    ...
    func bindData(){
        let index = tableview.selectedRow
        if index<0{return}
        noteTextfield.bind("value", to: _profile[index], withKeyPath: "note", options: [NSContinuouslyUpdatesValueBindingOption: true])
    }
